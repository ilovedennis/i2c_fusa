...
module i2c_reg_top
  ...
  // APB directly drives internal register bus
  assign reg_we     = psel_i && penable_i && pwrite_i;
  assign reg_re     = psel_i && penable_i && !pwrite_i;
  assign reg_addr   = paddr_i[AW-1:0];
  assign reg_wdata  = pwdata_i;
  assign reg_be     = 4'b1111;

  assign prdata_o   = reg_rdata;
  // single cycle access response
  assign pready_o   = 1'b1;
  assign pslverr_o  = reg_error | (RaclErrorRsp & racl_error_o.valid);
  ...
  assign racl_error_o.request_address = 32'(reg_addr);
  ...
  assign unused_policy_sel = ^racl_policies_i[0];
